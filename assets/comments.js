...
(function () {
    ...
    avatarStyle.textContent = `
        .ab-comment-avatar{overflow:hidden;color:#fff} 
...
.ab-avatar-fallback[hidden]{display:none!important}
    ...
    function fallbackAvatar() {
        return `<svg viewBox="0 0 32 32" aria-hidden="true" focusable="false"><circle cx="16" cy="10.5" r="6" fill="currentColor"/><path d="M4.5 27c.7-6.2 5.2-9.5 11.5-9.5S26.8 20.8 27.5 27c.08.72-.49 1.3-1.21 1.3H5.71C4.99 28.3 4.42 27.72 4.5 27Z" fill="currentColor"/></svg>`;
    }

    function avatarHtml(comment) {
        const url = comment?.profiles?.avatar_url;
        if (!url) return fallbackAvatar();
        return `<img src="${esc(url)}" alt="" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.hidden=false"><span class="ab-avatar-fallback" hidden>${fallbackAvatar()}</span>`;
    }
    ...
    async function loadComments(animeId, listEl, countEl) {
        const { data, error } = await client
            .from("comments")
            .select("id, created_at, updated_at, user_id, content, profiles:profiles!comments_user_profile_fkey(avatar_url)")
            .eq("anime_id", animeId)
        ...
        listEl.innerHTML = comments.map(comment => {
            ...
            return `
                <article class="ab-comment" data-comment-id="${esc(comment.id)}">
                    <div class="ab-comment-avatar">${avatarHtml(comment)}</div>
    ...
    async function render(animeId) {
        ...
        await refresh();
        window.addEventListener("anumbrix-auth-changed", refresh);
    }
